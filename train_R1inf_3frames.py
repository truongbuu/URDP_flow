...
def cal_W1(ssf, ssf1_hat, ssf1_MMSE, discriminator, discriminator_M, test_loader):
    ...
    for i, x in enumerate(iter(test_loader)):
        with torch.no_grad():
            ...
            x1 = x[:,:,0,...]
            with torch.no_grad():
                x2_bar = ssf1_MMSE(x2, x1)
                x2_hat = ssf1_hat(x2, x1)
...
def main():
    ...
    #Create folder
    #Create folder:
    folder_name='R1inf_3frames_dim_'+str(dim)+'|z_dim_'+str(z_dim)+'|L_'+str(L)+'|_single_bit'+str(single_bit)+'|lambda_gp_'+str(lambda_gp) \
    ...
    ssf1_MMSE = ScaleSpaceFlow(num_levels=1, dim=z_dim, stochastic=True, quantize_latents=True, L=L,single_bit=single_bit)
    ...
    ssf1_MMSE.motion_encoder.load_state_dict(torch.load(pre_path_mse+'/m_enc.pth'))
    ssf1_MMSE.motion_decoder.load_state_dict(torch.load(pre_path_mse+'/m_dec.pth'))
    ssf1_MMSE.P_encoder.load_state_dict(torch.load(pre_path_mse+'/p_enc.pth'))
    ssf1_MMSE.res_encoder.load_state_dict(torch.load(pre_path_mse+'/r_enc.pth'))
    ssf1_MMSE.res_decoder.load_state_dict(torch.load(pre_path_mse+'/r_dec.pth'))

    pre_path_plf=args.pre_path2
    ssf1_hat.motion_encoder.load_state_dict(torch.load(pre_path_plf+'/m_enc.pth'))
    ssf1_hat.motion_decoder.load_state_dict(torch.load(pre_path_plf+'/m_dec.pth'))
    ssf1_hat.P_encoder.load_state_dict(torch.load(pre_path_plf+'/p_enc.pth'))
    ssf1_hat.res_encoder.load_state_dict(torch.load(pre_path_plf+'/r_enc.pth'))
    ...
    for epoch in range(total_epochs):
        ...
        if epoch %10 == 0:
            ...
            +str(lambda_MSE) + " " + " P loss: " + str(cal_W1(ssf, ssf1_hat, ssf1_MMSE, discriminator, discriminator_M, test_loader))
            print (show_str)
